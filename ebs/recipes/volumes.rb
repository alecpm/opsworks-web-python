execute 'nvme-cli-ubuntu-14-ppa' do
  ...
  only_if { !node.normal['pretend_ubuntu_version'] && EbsVolumeHelpers.nvme_based? && !EbsVolumeHelpers.has_ebs_tooling? && platform?("ubuntu") && node[:platform_version] == "14.04" && node[:ebs][:devices].size != 0 }
end
