...
instances = (node[:cpu][:total].to_i * instance_data["per_cpu"].to_f).ceil if (node[:cpu][:total])
instances = (node[:opsworks][:instance][:backends].to_i * instance_data["per_cpu"].to_f/8).ceil if (!node[:cpu][:total] && (!node[:opsworks][:instance][:backends].nil? && !node[:opsworks][:instance][:backends].zero?))
...
if instance_data["enable_celery"]
  ...
  if host
    ...
    beat_cmd = 'beat'
    beat_cmd += ' ' + instance_data['beat_args'] if instance_data['beat_args']
    ...
    if instance_data['celerybeat']
      init_commands.push({'name' => "celerybeat", 'cmd' => 'bin/celery', 'args' => beat_cmd})
    end
  end
end
