...
if node.recipe?('plone_buildout::instances-setup')
  ...
  instances = (node[:opsworks][:instance][:backends].to_i * node["plone_instances"]["per_cpu"].to_f/2.0).ceil if (!node[:opsworks][:instance][:backends].nil? && !node[:opsworks][:instance][:backends].zero?)
